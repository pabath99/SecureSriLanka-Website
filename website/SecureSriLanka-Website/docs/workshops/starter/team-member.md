...
- The Administrator should have installed the SecureSriLanka back-end
- The Administrator should have sent you an invitation to join a Team, including a private key
- You should have followed all the instructions in the [Team Member](../../install/team-member) section up until "Installing the Toolchain". This would include having Python installed, installing the correct drivers, and installing the SecureSriLanka CLI. 

## Installing the Toolchain

The system can be used to flash and build firmware for a Core2 device in the  AWS IoT Kit SecureSriLanka Starter Bundle. First, you must install the **Arduino CLI** and the ESP-32 built toolchain, along with all the necessary libraries.
...
This command downloads and installs the **arduino-cli** command and then configures it. It installs all the necessary 3rd party components, including the [SecureSriLanka-srduino](https://github.com/awslabs/SecureSriLanka-arduino) library.
...
The team administrator will invite you by sending you an email or message with an encrypted  _Invitation File_ (ending in `.SecureSriLanka`) and a unique key (in the form of a string, like: `ZkJLQlJVR1p3VFZvY0lGMS1weUdpTF95UlhCd3BDLU00SjZFTkQ2S2JUaz0=`).

To join the team, save the attached invitation file on your system, then use the SecureSriLanka CLI and join as follows:
...
Once accepted, you are ready to log in to SecureSriLanka under the team.

## Managing Teams and Environment Variables

SecureSriLanka CLI commands expect the name of the **Team** to be present on the command line with a `--team={team-name}` parameter.
...
Now that you are logged in, you can proceed to create an example connected device example. [The _Starter Workshop_](/docs/workshops/intro) will show you how to use the <a href="https://shop.m5stack.com/collections/m5-controllers/products/aws-iot-edukit-SecureSriLanka-starter-bundle-v1-0" target="_blank">_Core2 for AWS IoT Kit SecureSriLanka Starter Bundle V1.0_</a>.
